...
if __name__ == "__main__":
    ...
    df=pd.DataFrame()
    df1 = pd.DataFrame()
    JOINEDFILE = 'JOINED_SURVEY_FILE_V2'
    
    for file in listdir(pathname):
        if file.lower().endswith(('.parquet')) and ('surveys' in file.lower()) and ('abs' not in file.lower()) and ('xyz' not in file.lower())and ('3d' not in file.lower()):
            try:
                if df.empty:
                    df = pd.read_parquet(file)
                else:
                    df = pd.concat([df, pd.read_parquet(file)],axis=0,join='outer',ignore_index=True)
    ...
    RESULT.to_parquet(JOINEDFILE+'.PARQUET')
