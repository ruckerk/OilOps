...
def CONSTRUCT_DB(DB_NAME = 'FIELD_DATA.db'):
    ...
    QRY = 'SELECT MAX(CAST(S.UWI AS INT), CAST(P.UWI AS INT)) AS UWI10, P.MONTH1, S.JOB_DATE, S.JOB_END_DATE, S.FIRST_PRODUCTION_DATE FROM SCOUTDATA AS S LEFT JOIN PRODUCTION_SUMMARY AS P ON CAST(S.UWI as INT) = CAST(P.UWI AS INT)'
