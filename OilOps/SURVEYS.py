...
##                        outdf_in['UWI'] = ReadUWI
            ...
            outdf_in = outdf_in.map(lambda x: str2num(x))
...
def survey_from_excel(file:(str,tuple), ERRORS:bool = True): #if True:
    ...
    if isinstance(outdf,pd.DataFrame):
        ...
        if not(UWI is None):            
            ...
            outdf = outdf.map(lambda x: str2num(x))
    ...
    for ffile in FLIST:#if 1==1:
        try:
            ...
            if ffile.lower().endswith(('.xls','xlsx','xlsm')):
                ...
                rdf = rdf.map(lambda x: str2num(x))
